...
object Par {
	 ...
	 * for controlling parallelism. We can always do `fork(map2(a,b)(f))` if we want the
	...
	def map2[A,B,C](a: Par[A], b: Par[B])(f: (A,B) => C): Par[C] =
		(es: ExecutorService) => {
			 ...
			 * It simply passes the `ExecutorService` on to both `Par` values, waits for
			 * the results of the Futures `af` and `bf`, applies `f` to them, and wraps them
			 * in a `UnitFuture`. In order to respect timeouts, we'd need a new `Future` implementation
			 * that records the amount of time spent evaluating `af`, then subtracts that time
	 ...
	 * this implies that we're losing out on some potential parallelism. Essentially, we're using two threads
	 * when one should suffice. This is a symptom of a more serious problem with the implementation, and
...
object Examples {
	 ...
	 * Unlike lists, these sequences provide an efficient `splitAt` method for dividing them into
